...
class Account(ABC):
    ...
    def _validate_channel_session(self, channel):
        ...
        if isinstance(channel, Channel):
            if channel.current_card != None and channel.current_card == self.__card:
                return True
            else :
                raise PermissionError("Invalid Card")
    ...
    def print_transactions(self):
        ...
        """
        print(f"\n--- History for {self.__account_no} ({type(self).__name__}) Balance: {self.__balance:.2f} ---")
    ...
    def _get_channel_id(self, channel):
        ...
        """
        if isinstance(channel, Channel):
            if hasattr(channel, "atm_no"):
                return channel.atm_no
            elif hasattr(channel, "edc_no"):
                return channel.edc_no
            elif hasattr(channel, "counter_no"):
                return channel.counter_no
            else : return "UNKNOWN"
    ...
    def deposit(self, channel, amount):
        ...
        if isinstance(channel, Channel):
            if amount > 0 and not isinstance(amount, str):
                if channel.authen:
                    if not isinstance(channel, Counter):
                        if self._validate_channel_session(channel):
                            pass
                        else: raise PermissionError("No Card")
                    self.__balance += amount
                    if hasattr(channel, "receive_cash"):
                        channel.receive_cash = amount
                    self._create_transaction("D", channel.get_type , channel.id, amount, self.__balance)
                    return "Deposit Successfully"
                else: raise PermissionError("Authenticate Unsuccessfully")
            else: raise ValueError("Invalid Amount of Money")
        ...
        if isinstance(channel, Channel):
            if amount > 0 and not isinstance(amount, str):
                    if channel.authen:
                        ...
                        if self._check_withdraw_limit(amount):
                            if not isinstance(channel, ATM_machine):
                                if self.__balance >= (amount + self.FEE):
                                    pass
                                else : raise ValueError("Not enough money for FEE")
                            else:
                                if self.__balance - amount - self.FEE >= self.__card.ANNUAL_FEE:
                                    pass
                                else : raise ValueError("Not enough money for FEE and ANNUAL_FEE")
                                if channel.has_sufficient_cash(amount):
                                    channel.dispense_cash = amount
                                    pass
                                else : raise ValueError("Not Enough Cash")
                            if not isinstance(channel, Counter):
                                if self._check_daily_limit(amount):
                                    pass
                                else: raise ValueError("Exceed Daily Limit")
                            self.__balance -= amount + self.FEE
                            self.__daily += amount
                            if not isinstance(channel, Counter):
                                self._create_transaction("F", channel.get_type, channel.id, self.FEE, self.__balance)
                            self._create_transaction('W', channel.get_type, channel.id, amount, self.__balance)
                            print("Withdraw Successfully")
                        else: raise ValueError("Exceed withdraw limit")                          
                    else: raise PermissionError("Authenticate Unsuccessfully")
    ...
    def transfer(self, channel, amount, target_account):
        ...
        9. แสดงข้อความสำเร็จ
        """
        if isinstance(channel, Channel) and isinstance(target_account, Account):
            if target_account.account_no != self.__account_no:
                if amount > 0 and not isinstance(amount, str):
                        if channel.authen:
                            if not isinstance(channel, Counter):
                                if self._validate_channel_session(channel):
                            ...
                            if self._check_withdraw_limit(amount):
                                if not isinstance(channel, ATM_machine):
                                    if self.__balance >= (amount + self.FEE):
                                        pass
                                    else : raise Exception("Not enough money for FEE")
                                else:
                                    if self.__balance - amount - self.FEE >= self.__card.ANNUAL_FEE:
                                        pass
                                    else : raise Exception("Not enough money for FEE and ANNUAL_FEE")
                                if isinstance(channel, ATM_machine):
                                    self._check_daily_limit(amount)
                                    
                                if isinstance(channel, EDC_machine):
                                    channel._check_limit_daily(amount)
                                    self.__cashback = channel.refund(self.__card, amount)
                                    self.__balance += self.__cashback
                                    channel.current_card.recieve_cashback(self.__cashback)
                                    self._create_transaction("I", channel.get_type, channel.id, self.__cashback, self.__balance)
                                self.__balance -= amount + self.FEE
                                target_account.receive_transfer(amount, channel, self.__account_no)
                                self._create_transaction("TW", channel.get_type, channel.id, amount, self.__balance, target_account.account_no)
                                
                                print("Transfer Successfully")
                            else: raise Exception("Exceed withdraw limit")    
                                            
                        else: raise PermissionError("Authenticate Unsuccessfully")
                else: raise ValueError("Invalid Amount of Money")
            else: raise ValueError("Same Account")               
    ...
    def receive_transfer(self, amount, channel, source_acc_no):
        ...
        self.__balance += amount
        self._create_transaction("TD", channel.get_type, channel.id, amount, source_acc_no)
...
class SavingAccount(Account):
    ...
    def calculate_interest(self):
        ...
        print("Calculated Interest Successfully")
        return interest
    ...
    def _check_withdraw_limit(self, amount):
        ...
        if amount > self.WITHDRAW_LIMIT_PER_TRANSACTION:
            raise ValueError("Amount Exceed Withdraw Limit)") 
        else: return True
        
    def _check_daily_limit(self, amount):
        if self._Account__card.get_card_type() == "Premium Card":
            self.WITHDRAW_LIMIT_PER_DAY = 100000
        if  self._Account__daily + amount > self.WITHDRAW_LIMIT_PER_DAY:
...
class FixedAccount(Account):
    ...
    def __init__(self, account_no, user, amount, term_months=12):
        ...
        self.__maturity_date = self.__start_date + timedelta(days=self.__term_months*30)
...
class CurrentAccount(Account):
    ...
    def _check_daily_limit(self, amount):
        if self.__card.get_card_type() == "Premium Card":
            self.WITHDRAW_LIMIT_PER_DAY = 100000
        if  self.__daily + amount > self.WITHDRAW_LIMIT_PER_DAY:
            raise ValueError("Exceed Daily Limit")
        else: return True
    ...
    def charge_annual_fee(self, account):
        ...
        if isinstance(account, Account):
            if account.amount >= self.ANNUAL_FEE:
                ...
                account._create_transaction('F', 'SYSTEM', 'ANNUAL_FEE(ATM)', self.ANNUAL_FEE, account.amount)
...
class Debit_Card(Card):
    ...
    def recieve_cashback(self, amount):
...
class ShoppingCard(Debit_Card):
    ...
    def charge_annual_fee(self, account):
        if isinstance(account, Account):
            if account.amount >= self.ANNUAL_FEE:
                account.amount = -self.ANNUAL_FEE
                account._create_transaction('F', 'SYSTEM', 'ANNUAL_FEE(Shopping)', self.ANNUAL_FEE, account.amount)

  
        

class PremiumCard(Debit_Card):
    ANNUAL_FEE = 500
    CASHBACK_RATE = 0.02
    WITHDRAW_LIMIT_PER_TRANSACTION = 40000
    DAILY_LIMIT = 100000
    ...
    def charge_annual_fee(self, account):
        if isinstance(account, Account):
            if account.amount >= self.ANNUAL_FEE:
                account.amount = -self.ANNUAL_FEE
                account._create_transaction('F', 'SYSTEM', 'ANNUAL_FEE(Premium)', self.ANNUAL_FEE, account.amount)
...
class ATM_machine(Channel):
    ...
    def authenticate(self, card, pin):
        ...
        """
        if isinstance(card, ATM_Card):
            if self.insert_card(card, pin):
                return True
            else : 
                return False
        else :return False
...
class EDC_machine(Channel):
    def __init__(self, edc_no, account):
    ...
    def authenticate(self, card, pin):
        ...
        TODO: เรียก insert_card(card, pin) และ return ผลลัพธ์
        """
        if isinstance(card, Debit_Card):
            if self.swipe_card(card, pin):
                return True
            else : 
                return False
        else :return False
    ...
    def pay(self, target, amount):
        target.transfer(self, amount, self.__account)
    ...
    def _check_limit_daily(self, amount):
        if self.__daily+amount > 40000:
            raise Exception("Exceed Daily Limit 40,000/day")
        self.__daily += amount

    def refund(self, card, amount):
        if card.get_card_type() == "Shopping Card" and amount >= 1000:
            return 0.01*amount
        elif card.get_card_type() == "Premium Card":
            return 0.02*amount
        return 0
...
class Counter(Channel):
    ...
    def verify_identity(self, account, citizen_id):
        if isinstance(account, Account):
            if account.user.get_citizen_id == citizen_id:
                self.__current_account = account 
                self.__authen = True
            else : self.__authen = False
...
class Bank:
    ...
    def search_account_from_card(self, card_no):
        ...
        """
        for index, item in enumerate(self.__user_list):
            if item.search_account_from_card(card_no):
                return item
        return None
...
class User:
    ...
    def search_account_from_card(self, card_no):
        ...
        for index, item in enumerate(self.__account_list):
            if item.card != None and item.card.card_no == card_no:
                return item
        return None
...
class Transaction:
    ...
    def __str__(self):
        ...
        """
        return f"{self.__type}-{self.__channel_type}:{self.__channel_id}-{self.__amount}-{self.__balance}[-{self.__target}]"
